...
namespace pose_pid
{
        ...
        allowed_area(get_parameter("allowed_area").as_double_array())
  {
    ...
    _subscription_target = this->create_subscription<geometry_msgs::msg::PointStamped>(
        "target_pose",
        _qos,
    ...
    _pub_timer = this->create_wall_timer(
        ...
        [this]
        { if(is_autonomous)_publisher_callback(); });
  }
  ...
  void PosePID::_subscriber_callback_target(const geometry_msgs::msg::PointStamped::SharedPtr msg)
  {
    ...
    target_->point.y = constrain(target_->point.y, allowed_area.at(2), allowed_area.at(3));
    target_->point.z = constrain(target_->point.z, -pi<double>(), pi<double>());
    this->target = target_;
    RCLCPP_INFO(this->get_logger(), "目標姿勢入力  x:%f  y:%f  theta:%f", target->point.x, target->point.y, target->point.z);
  }
