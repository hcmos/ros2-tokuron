...
void Controller::_subscriber_callback_joy(const sensor_msgs::msg::Joy::SharedPtr msg){
    ...
    if(!is_autonomous){
        ...
        msg_vel->twist.linear.x = linear_max_vel * msg->axes[static_cast<int>(Axes::L_y)];
        msg_vel->twist.linear.y = linear_max_vel * msg->axes[static_cast<int>(Axes::L_x)];
        msg_vel->twist.angular.z = angular_max_vel * msg->axes[static_cast<int>(Axes::R_x)];
